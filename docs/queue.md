...
- **可见性问题**：需要手动设计超时/重试策略，避免任务永久卡住。

## 专业升级方案：PGMQ

如果你需要更健壮的任务队列功能，建议直接使用 [PGMQ](https://github.com/tembo-io/pgmq)。本示例中的 `SKIP LOCKED` 是 PGMQ 核心逻辑的简化实现。

### 为什么选择 PGMQ 替代手动 SKIP LOCKED

| 特性 | 手动 SKIP LOCKED (本示例) | PGMQ 扩展 |
|------|-------------------------|----------|
| **核心逻辑** | `FOR UPDATE SKIP LOCKED` | 同左，但经过高度优化 |
| **可见性超时** | 需要手动维护 `locked_at` | 内置支持，自动处理超时重试 |
| **代码量** | 需要编写复杂的 SQL UPDATE | 简单的函数调用：`read()`, `delete()` |
| **死信队列** | 需要手动实现 | 支持归档与消息保留策略 |
| **延迟队列** | 手动对比 `scheduled_at` | 内置支持 |
| **管理工具** | 无 | 提供多语言客户端与管理函数 |
...
-- 2. 生产者：入队任务
SELECT pgmq.send('task_queue', '{"task_type": "email", "id": 123}'::jsonb);

-- 3. 消费者：领取任务（锁定 60 秒）
-- 如果 60 秒内没 delete，任务会自动回到队列重试
SELECT * FROM pgmq.read('task_queue', 60, 1);
...
- **简单场景**：使用本示例的 `SKIP LOCKED` 方案即可，代码透明且无插件依赖。
- **复杂场景**：需要严格的任务超时回收、死信队列、历史重放时，推荐 **PGMQ**。
